...
// Strict local secure storage layer - force memory reset to trigger auto logout on reload parameter settings
...
// ==========================================
// 🌟 SINGLE-PAGE ROUTING CONSOLE PORTS
// ==========================================
...
// 🌟 ADMIN CONTROL LOGIC OPERATION PORTS (Direct Node Publishing & Dynamic Selection Updates)
...
window.triggerAdminPostEditSelectMode = function(id) {
    ...
    // Extract description strings fallback configuration rules
    const parser = new DOMParser();
    const docHtml = parser.parseFromString(post.description || "", 'text/html');
    const mainText = docHtml.querySelector('.text-sm')?.innerHTML || post.description || "";
    document.getElementById('aDesc').value = mainText;
};
...
function bootstrapApplicationEngine() {
    ...
    // Strict clean user logout session validation mapping
    signOut(auth); 
            ...
            if(data.approvalStatus === 'Pending') {
                approvalQueueHTML += `
                    <div class="p-3 bg-white border rounded-xl space-y-2 text-xs shadow-sm">
                        ...
                        <div class="flex gap-2"><button onclick="window.approvePostItemNode('${id}')" class="bg-emerald-600 text-white px-2 py-1 rounded font-bold text-[11px]">Approve</button><button onclick="window.rejectPostItemNode('${id}')" class="bg-rose-600 text-white px-2 py-1 rounded font-bold text-[11px]">Reject</button></div>
                    </div>`;
...
window.addEventListener('DOMContentLoaded', bootstrapApplicationEngine);import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
import { getFirestore, collection, addDoc, doc, setDoc, onSnapshot, updateDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendPasswordResetEmail, signOut } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js";
import { getMessaging, getToken } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-messaging.js";
